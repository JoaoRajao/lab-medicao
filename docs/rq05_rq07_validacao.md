...
```bash
duckdb -c "select * from read_parquet('github_ingest/data/parquet/repositories.parquet') limit 5;"
```
...
- Arquivo de referencia: `github_ingest/config/popular_languages.json`
...
```sql
select *
from read_parquet('github_ingest/data/parquet/repositories.parquet');
```
