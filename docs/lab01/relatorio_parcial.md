...
- Coleta via API GraphQL do GitHub, usando query e script proprios do grupo (`labs/lab01_repos_populares/ingest.py`), sem bibliotecas de terceiros para consumir a API.
...
- Saidas geradas: DuckDB (`data/warehouse.duckdb`), Parquet (`labs/lab01_repos_populares/data/parquet/repositories.parquet`) e CSV (`labs/lab01_repos_populares/data/csv/repositories.csv`).
- Modelagem em camadas via dbt: staging (`stg_lab01_repos_populares`) e gold (uma tabela por RQ, `models/gold/lab01/`), com testes `not_null`/`unique` e testes de consistencia especificos para RQ05-07.
- Fonte de referencia para "linguagens populares" (RQ05 e RQ07): GitHub Octoverse 2025 (`labs/lab01_repos_populares/config/popular_languages.json`), mantida constante em todo o laboratorio.

## 3. Resultados por RQ

### RQ01 - Idade dos repositorios (`gold_lab01_rq01_age`)
...
### RQ02 - Pull requests aceitas (`gold_lab01_rq02_pull_requests`)
...
### RQ03 - Releases (`gold_lab01_rq03_releases`)
...
### RQ04 - Dias desde a ultima atualizacao (`gold_lab01_rq04_updates`)
...
- **Snapshots de fechamento de sprint:** script proprio (`shared/kanban/export_project_snapshot.py`), reaproveitando a query GraphQL da Parte 1, exportando itens + status atual para CSV a cada sprint (`data/kanban_snapshots/`). Snapshots disponiveis ate o momento:
